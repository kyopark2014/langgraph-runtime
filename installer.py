...
def _ecs_agent_runtime_resource_arns() -> List[str]:
    ...
    Root installer project_name can differ from Agent Runtime installer naming.
    Include project/git names, every runtime_agent/*/ directory name, and any
    explicit agent_runtime_arn from local configs.
    """
    root = os.path.dirname(os.path.abspath(__file__))
    app_config = _read_local_json_config(os.path.join(root, "application", "config.json"))

    runtime_configs: List[Dict] = []
    runtime_agent_root = os.path.join(root, "runtime_agent")
    name_candidates = {
        agent_runtime_name(project_name),
        agent_runtime_name(git_name),
    }
    try:
        for entry in sorted(os.listdir(runtime_agent_root)):
            entry_path = os.path.join(runtime_agent_root, entry)
            if not os.path.isdir(entry_path) or entry.startswith("."):
                continue
            name_candidates.add(agent_runtime_name(entry))
            cfg = _read_local_json_config(os.path.join(entry_path, "config.json"))
            if cfg:
                runtime_configs.append(cfg)
    except OSError:
        pass

    for config in [app_config, *runtime_configs]:
                ...
                "Statement": [
                    {
                        ...
                        # See _ecs_agent_runtime_resource_arns.
...
def _load_runtime_agent_config(runtime_type: str = "langgraph") -> Dict[str, str]:
...
def update_skills_list_from_directory() -> List[str]:
    """Rewrite application/skills.list from runtime_agent/langgraph/skills/*/SKILL.md names.

    Runtime no longer ships a langgraph/skills.list; per-user lists live under
    ...
    """
    skills_dir = os.path.join(_project_root(), "runtime_agent", "langgraph", "skills")
...
def sync_application_capability_lists() -> None:
    ...
    - skills.list: rebuild from runtime_agent/langgraph/skills/*/SKILL.md
    - mcp.list: copy from runtime_agent/langgraph/mcp.list
    """
    update_skills_list_from_directory()

    src = os.path.join(_project_root(), "runtime_agent", "langgraph", "mcp.list")
    ...
    logger.info("  ✓ Overwrote application/mcp.list from runtime_agent/langgraph/mcp.list")
...
def build_and_push_docker_image(
    ...
    # Refresh application capability lists from runtime before image build.
...
def create_alb_listener_with_target_group(
    ...
    """Create ALB listener that forwards only when the CloudFront origin header matches."""
